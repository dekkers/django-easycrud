...
def easycrud_urlpatterns():
    model_list = [m for m in apps.get_models() if issubclass(m, EasyCrudModel)]
    url_list = []
    ...
    for model in model_list:
        ...
        url_list.append(url(r'^%s/(?P<pk>\d+)/$' % name, DetailView.as_view(model=model), name='%s_detail' % name))
        if model._easycrud_meta.inline_models:
            ...
            if model.has_update:
                url_list.append(url(r'^%s/(?P<pk>\d+)/update/$' % name, UpdateWithInlinesView.as_view(model=model, inlines=inlines), name='%s_update' % name))
        else:
            if model.has_create:
                url_list.append(url(r'^%s/create/$' % name, CreateView.as_view(model=model), name='%s_create' % name))
            if model.has_update:
                url_list.append(url(r'^%s/(?P<pk>\d+)/update/$' % name, UpdateView.as_view(model=model), name='%s_update' % name))
        if model.has_delete:
            url_list.append(url(r'^%s/(?P<pk>\d+)/delete/$' % name, DeleteView.as_view(model=model), name='%s_delete' % name))
